...
def send_summary():
    ...
    try:
        ...
        if r.json().get("code") == 0:
            print(f"[通知] [OK] 微信推送成功")
        else:
            print(f"[通知] [FAIL] 推送失败: {r.json()}")
    except Exception as e:
        print(f"[通知] [FAIL] 异常: {e}")
...
def check_one(account: dict, today: date) -> None:
    ...
    for rec in records:
        ...
        print(f"    上午: {'[OK] '+morning_time if morning_ok else '[FAIL] 未打卡'}")
        print(f"    下午: {'[OK] '+afternoon_time if afternoon_ok else '[FAIL] 未打卡'}")
        if night_time:
            print(f"    晚上: [OK] {night_time}")
        ...
        if not morning_ok:
            ...
            detail.append(f"上午: [ERR] 未打卡")
        else:
            detail.append(f"上午: [OK] {morning_time}")
        if not afternoon_ok:
            missing.append("下午(下班)")
            detail.append(f"下午: [ERR] 未打卡")
        else:
            detail.append(f"下午: [OK] {afternoon_time}")
        if night_time:
            detail.append(f"晚上: [OK] {night_time}")
...
def _status_text(val: int) -> str:
    return {0: "[ERR] 未打卡", 1: "[OK] 正常", 2: "!! 迟到", 3: "— 休息"}.get(val, f"未知({val})")
